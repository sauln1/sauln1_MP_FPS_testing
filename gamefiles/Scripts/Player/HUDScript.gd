...
@onready var reticleStatsBar = %HealthBar_Reticle
@onready var detailedStatsBar = %HealthBar_Detailed
...
func displayCurrentState(currentState):
	...
	if currentState == 8:
		stateSignification = "Clambering"
	if currentState == 9:
		stateSignification = "Falling"
	if currentState == 10:
		stateSignification = "Dashing"
	if currentState == 11:
		stateSignification = "Dead"
